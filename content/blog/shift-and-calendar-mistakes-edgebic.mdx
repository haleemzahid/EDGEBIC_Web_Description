...
faqQuestions:
  ...
  - question: "Should you shrink a shift to account for daily lost time?"
    answer: "Only when the loss really is every day. A shift shortened to cover an occasional meeting costs those hours every single day and hides the reason, so a year later nobody can explain why the day is short. A recurring partial holiday charges each shift only its own overlap with the window, applies plant-wide from one record, and documents itself. Reserve shift edits for changes to the actual working pattern."
  ...
  - question: "What is the safest way to leave planning headroom in a schedule?"
    answer: "Put it in the calendar, not in hope. Enter shift hours net of time the plant reliably loses, record recurring maintenance as downtime so it is subtracted before the instance multiplier, or use per-day overrides on the specific days that differ. All three are honored identically by the scheduler and the dashboards, and all three leave a record explaining the number, which a mental buffer never does."
qaQuestions:
...
**The fix.** Model it as a recurring partial holiday for the meeting window. Each shift is charged only its own overlap:
...
One record, correct per shift, self-documenting, and it repeats automatically. Reserve shift edits for genuine changes to the working pattern.
...
**The fix.** Override each shift you intend to run. And remember the priority rule: a per-day override beats a date-range override on any date both cover, which means a single day overridden inside a retooling week comes out of that week's budget entirely.
...
**The fix.** Use a per-day capacity override on the specific Saturdays you actually intend to run, entered per shift, with a reason. That is not a workaround: it is the better model, because occasional overtime should be a deliberate decision with an audit trail rather than a permanent calendar assumption. For a stretch of weekend work, a date-range override makes those days schedulable across the window.
...
## 9. Downtime Longer Than the Shift

**The mistake.** A five-hour maintenance window recorded against a four-hour shift, usually because the window spans two shifts and was entered once.

**Why it is wrong.** The capacity result is floored at zero rather than going negative, so the shift silently returns no hours instead of complaining. Nothing at data entry prevents the combination.

**The symptom.** A shift that goes completely dark on a recurring weekday with no holiday and no obvious cause.

**The fix.** Record downtime per shift, sized to fit inside it. If a window genuinely spans two shifts, enter the portion that belongs to each. And when a shift goes to zero unexpectedly, check downtime before anything else: the full ordered walk is in [how EDGEBIC resolves capacity day by day](/blog/how-edgebic-resolves-capacity-day-by-day).

## Two Habits That Prevent Most of This

**Keep the calendar honest rather than optimistic.** A resource planned to 100% of the theoretical clock has no recovery room, and the first breakdown propagates through every downstream date. That reasoning is the same one behind buffer management in the [Theory of Constraints and APS literature](https://www.ascm.org). Express the headroom explicitly (real shift hours, recorded downtime, per-day overrides) so the number carries a reason. The measurement side of the same idea is in [the capacity utilization KPI](/blog/capacity-utilization-kpi).
