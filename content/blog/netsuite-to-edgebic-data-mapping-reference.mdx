---
title: "NetSuite to EDGEBIC: The Data Mapping Reference"
description: "NetSuite data export scheduling field by field: map items, work centers, routings, work orders and labor into EDGEBIC masks, with CSV and upsert rules."
...
keywords:
  - NetSuite data export scheduling
  - NetSuite field mapping scheduling
  - NetSuite routing export
  - import mask field reference
  - NetSuite work order export
  - cloud ERP data mapping
targetPhrase: "NetSuite data export scheduling"
wordCount: 2199
readingTime: 11
faqQuestions:
  - question: "Which columns must a NetSuite export contain before EDGEBIC will import it?"
  ...
  - question: "Can EDGEBIC read a NetSuite CSV with quoted text and embedded commas?"
  ...
  - question: "What happens to dates in a locale-specific NetSuite export?"
    answer: "Date cells are parsed against the invariant format first and the machine's regional format second, so a locale-specific export usually lands correctly without pre-processing. If a specific cell still cannot be parsed, that row alone is marked Failed with the reason recorded in the per-run log while the rest of the file continues, so one malformed date never blocks 500 good rows."
  - question: "In what order should NetSuite files be imported?"
    answer: "Dependency order: items first, then work centers, then routings, then open work orders, then labor. Routings reference both items and work centers, and work orders reference items, so loading upstream data first stops the auto-create behavior from inventing thin placeholder records. Auto-create is on by default but a record it creates carries only the name the file gave it."
qaQuestions:
  - question: "Our exports come out with a header row for some lists and without one for others, because a few are scheduled extracts. Do we have to normalize them first?"
    answer: "No, because the mask handles both shapes. With header-present on, you map by dragging column headings, which is what you want for a hand-run export. With it off, you map each field by its 1-based column position instead, which is the mode for a raw scheduled extract with no headings. The position mapping is stored on the mask permanently, so the weekly routine is identical either way: pick the mask, press Do It, read the counts. The one discipline that matters for the positional masks is that the extract's column order must stay fixed, since a reordered column silently maps the wrong data rather than failing."
...
Two entity types have no NetSuite origin and come from a spreadsheet you write: **Shift** (one row per shift with a start and end pair per weekday, a blank pair meaning the day is off) and **PlantHoliday** (closures by name and date). Together they define the calendar every scheduled date is computed against, which makes them the ten minutes with the widest effect in the whole load.
...
Two rules follow. All of one item's steps must travel in one file, because a run wipes and recreates that item's steps on first reference, so splitting one item across two runs means the second wipe deletes the first run's work. And re-importing is safe for work orders in progress, because every scheduled job carries a frozen snapshot of the routing it was planned with.
...
| `Job_Number` | | Your NetSuite work order number carried across |
...
Two mask options handle the case where an export carries only one date: one treats the job date as the due date, the other derives the job date backward from the due date. Set whichever matches your export once, on the mask.
...
For the questions that come up before a project starts, see the [NetSuite integration FAQ](/blog/edgebic-netsuite-integration-faq). The [SAP](/blog/sap-to-edgebic-data-mapping-reference) and [Sage](/blog/sage-to-edgebic-data-mapping-reference) references show how little changes from one platform to the next, and the [ERP scheduling add-on page](/erp-scheduling-add-on) frames the category.

<Accordion title="Which columns must a NetSuite export contain before EDGEBIC will import it?">
Each entity type has a short mandatory list. Products need a product id. Work centers need a work center id. Routings need end product, step name, an operation flag, and hours required. Work orders need product, quantity, and a job date. Labor rows need job number, work center id, and the actual date. Everything else is optional, and an unmapped mandatory field aborts the run before any row is read.
</Accordion>

<Accordion title="Can EDGEBIC read a NetSuite CSV with quoted text and embedded commas?">
Yes. The import mask has a text-enclosed-in-quotes setting for RFC-4180 style quoting, so a quoted description containing a comma parses as one value instead of splitting into phantom columns. The delimiter itself is also a mask setting, covering comma, semicolon, tab, and space files, so the export needs no cleanup pass before it is imported.
</Accordion>

<Accordion title="What happens to dates in a locale-specific NetSuite export?">
Date cells are parsed against the invariant format first and the machine's regional format second, so a locale-specific export usually lands correctly without pre-processing. If a specific cell still cannot be parsed, that row alone is marked Failed with the reason recorded in the per-run log while the rest of the file continues, so one malformed date never blocks 500 good rows.
</Accordion>

<Accordion title="In what order should NetSuite files be imported?">
Dependency order: items first, then work centers, then routings, then open work orders, then labor. Routings reference both items and work centers, and work orders reference items, so loading upstream data first stops the auto-create behavior from inventing thin placeholder records. Auto-create is on by default but a record it creates carries only the name the file gave it.
</Accordion>
