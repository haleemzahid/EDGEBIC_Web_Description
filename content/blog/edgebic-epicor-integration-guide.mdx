...
The jobs import carries product, quantity, reference, and dates. Rows that share an order reference group under one sales order with jobs auto-numbered `{Ref}-{line}`. And one deliberate rule governs everything: **imports never schedule.** Imported jobs land as unscheduled demand; you then run the scheduler (typically new-jobs-only, so fresh work plans around existing commitments). Data movement and planning stay separate so that an import can never silently rearrange your floor.
