...
### Which Pattern EDGEBIC Uses

EDGEBIC by User Solutions targets Patterns 1 and 3 deliberately: a database read where the ERP database is reachable, and file exchange everywhere else. Both land in the same place, a saved import mask that remembers your column mapping and re-runs on demand, on a timer, or when a watched file changes. EDGEBIC does not ship per-ERP API connectors, and that is the point of the design: no certification step before you can start, no custom API development on the critical path, and no integration that stops working when your ERP vendor ships a major upgrade. See [how EDGEBIC import masks work](/blog/edgebic-import-masks-explained).
