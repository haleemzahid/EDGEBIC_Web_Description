...
<Image
  src="/marketing/blog/what-if-analysis-production-scheduling/hero.webp"
  alt="Manufacturing scheduler comparing two production schedule scenarios side by side on dual monitors"
  width="768"
  height="432"
...
3. **Reschedule** — the [finite capacity scheduling](/blogs/finite-vs-infinite-capacity-scheduling) engine recalculates the entire schedule with the change applied
4. **Evaluate the impact** — review which jobs shift, which deliveries are affected, and how [KPIs](/blogs/production-scheduling-kpis) change
5. **Decide and act** — accept the scenario and apply it to the live schedule, modify it, or reject it

The entire process takes seconds to minutes in modern scheduling software. In [Excel](/blogs/production-scheduling-excel-fails), the same exercise takes hours — if it is feasible at all.
...
This information is available in minutes instead of hours, enabling faster and better response to disruptions. See our guide on [real-time production scheduling](/blogs/real-time-production-scheduling) for more on disruption response.
...
- Which resources would become new [bottlenecks](/blogs/production-bottleneck-identification)
...
Should you schedule by earliest due date or critical ratio? [Forward or backward](/blogs/forward-vs-backward-scheduling)? Run the same job set under different [scheduling methods](/blogs/production-scheduling-methods) and compare the results:
...
**A finite capacity scheduling engine.** The analysis is only as good as the scheduling model. If the model does not respect [real constraints](/blogs/finite-vs-infinite-capacity-scheduling), the scenarios will be misleading.
...
**Visual comparison.** Side-by-side [Gantt chart](/blogs/drag-and-drop-scheduling-pros-cons) comparison of scenarios makes the impact immediately visible. [EDGEBI](/edgebi) provides this visual comparison alongside RMDB's scheduling engine.
