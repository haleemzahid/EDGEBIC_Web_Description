...
3. **Check the shift calendar and the per-day overrides.** A gap that starts at 16:00 and ends at 08:00 is the night, not a defect. A gap that lines up with a shortened day is usually a capacity override someone entered for maintenance, which shows in the machine's per-day capacity view with its reason attached.
