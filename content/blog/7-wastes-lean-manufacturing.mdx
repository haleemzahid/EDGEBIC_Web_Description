...
**How to reduce it**: Create manufacturing cells that group operations by product family. Use [value stream mapping](/blogs/value-stream-mapping-guide) to visualize material flow paths. Locate related operations adjacent to each other rather than grouping machines by type.
...
**How to reduce it**: Implement pull systems like [Kanban](/blogs/kanban-system-manufacturing) to control WIP levels. Use finite capacity scheduling to release work orders only when downstream capacity is available. Reduce batch sizes to decrease the total inventory in the production pipeline.
...
**Real-world example**: A defense subcontractor mapped their value stream for a missile component family and discovered that total processing time was 6.2 hours spread across 5 operations, but average lead time was 18 working days. The parts spent 97% of their time waiting. By implementing [lean scheduling techniques](/blogs/lean-scheduling-techniques) with RMDB to synchronize operations and reduce batch sizes, they cut lead time to 5 days.
...
**How to reduce it**: Review customer specifications carefully — do not assume tighter is better. Implement [poka-yoke (error-proofing)](/blogs/poka-yoke-error-proofing) to prevent defects rather than inspecting for them. Challenge every operation: does the customer value this step? Would they pay extra for it?
...
**How to reduce it**: Implement [poka-yoke](/blogs/poka-yoke-error-proofing) devices to prevent errors. Use [standard work](/blogs/standard-work-manufacturing) to ensure every operator follows the proven best method. Apply [PDCA cycles](/blogs/pdca-cycle-manufacturing) to investigate and permanently fix root causes. Track first-pass yield as a [key lean KPI](/blogs/lean-manufacturing-kpis).
...
[Gemba walks](/blogs/gemba-walks-guide) take leadership to the production floor to observe processes firsthand. Walk the path a part takes from raw material receipt to shipment. Time each operation, each transport step, and each queue. Document what you see without trying to fix anything yet.

### Step 2: Map the Value Stream

[Value stream mapping](/blogs/value-stream-mapping-guide) creates a visual representation of material and information flow for a product family. It distinguishes value-added time from non-value-added time and makes waste impossible to ignore. A typical first value stream map reveals that 90% or more of lead time is waste.
...
Combined with [EDGEBI analytics](/edgebi), you gain real-time visibility into where waste accumulates, enabling [continuous improvement](/blogs/kaizen-events-guide) driven by data rather than guesswork.
