...
2. **Set machine counts honestly.** Four presses is four instances, and taking one down for a rebuild is a calendar entry rather than a count edit.
...
5. **Attach recurring downtime** for shift-change routines and preventive maintenance so those hours stop being invisible.
...
It is also the fastest diagnostic you have. If a day's figure looks wrong, the bucket that produced it is where to look: the shift definition, its downtime, a closure, an override, or the machine count. Every one of those is visible, and none of them requires reading the schedule to work out.
