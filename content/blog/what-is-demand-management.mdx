...
Demand also comes from several origins, and labeling each one keeps the plan auditable. A build might trace to a confirmed sales order, a reorder rule that fired when stock dropped, a forecast, a master schedule commitment, or a planner's manual decision. Recording the source of every order lets a planner see why it exists and lets the reconciliation logic treat firm and forecast demand correctly.
