...
The one thing matching cannot absorb is a genuinely different string. An extra space, a swapped separator, or a leading zero present in one export and absent in another reads as a new identifier and creates a new item. Keeping the master clean is therefore an upstream discipline: choose one canonical form for the part number in the ERP and export it consistently.
