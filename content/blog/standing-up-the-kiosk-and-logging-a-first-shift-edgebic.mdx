...
faqQuestions:
  ...
  - question: "Does the kiosk timestamp come from the terminal or the server?"
    answer: "From the server, always. The operator taps a button and the system records the server's current time, so the terminal never sends a clock value. This matters after a crash or power cycle: on restart the kiosk detects the still-open punch and resumes the elapsed timer from the server's original open time, so no hours are lost and no client-clock drift creeps in. It also means an operator cannot accidentally backdate a punch by changing the terminal's time."
...
qaQuestions:
  ...
  - question: "The tablet rebooted mid-job during our first shift. Did we lose the hours the operator had logged?"
    answer: "No. On startup the kiosk checks for a punch that was open before the restart and, finding one, restores the screen to match it and resumes the elapsed timer from the server's original open time. Because punches are stamped server-side, the clock kept running while the terminal was down, so the elapsed time shown may look larger than the operator expected, and that is correct: the punch covers real wall-clock time. The logged pieces and the open run punch are intact, so the operator just keeps going."
---
...
2. **Every punch is server-stamped.** The terminal never sends a time, so the record survives crashes, wrong clocks, and reboots.
...
**Recover from a crash.** Power-cycle the terminal mid-run. On restart the kiosk detects the open punch, restores the running state, and resumes the elapsed timer from the server's original open time, so nothing is lost.
...
**Watch a preventive-maintenance banner.** If the machine's PM policy fields are set, the idle screen shows an approaching or overdue banner derived from the last PM date plus the interval, so the operator sees it before starting the next job.
