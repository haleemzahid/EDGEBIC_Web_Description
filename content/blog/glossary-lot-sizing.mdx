...
**Lot sizing** is the process of determining the quantity of an item to order or produce in a single batch. In [Material Requirements Planning (MRP)](/blogs/glossary-material-requirements-planning), the lot-sizing rule tells the system how to group [net requirements](/blogs/glossary-net-requirements) into [planned orders](/blogs/glossary-planned-order) — whether to order exactly what is needed, combine multiple periods of demand, or use a formula-based quantity.
...
**[Lot-for-Lot (LFL)](/blogs/glossary-lot-for-lot)** — Orders exactly the net requirement each period. Zero excess inventory but maximum ordering/setup frequency.

**[Economic Order Quantity (EOQ)](/blogs/glossary-economic-order-quantity)** — Uses the classic formula to balance ordering and holding costs. Works best for stable, predictable demand.
...
**Interacts with [safety stock](/blogs/glossary-safety-stock) policy.** Lot-sizing decisions should be made in conjunction with safety stock levels. Increasing lot sizes inherently provides a buffer; reducing lot sizes may require higher safety stock to maintain service levels.

---

## Related Terms

- **[Lot-for-Lot](/blogs/glossary-lot-for-lot)** — The lot-sizing method that orders exactly what is needed each period, minimizing carrying cost.
- **[Economic Order Quantity (EOQ)](/blogs/glossary-economic-order-quantity)** — The formula-based method that calculates the cost-optimal order quantity.
- **[Net Requirements](/blogs/glossary-net-requirements)** — The per-period demand figures that lot-sizing rules act upon to generate planned orders.
