...
faqQuestions:
  - question: "Why does a work center show zero capacity in my scheduling software?"
    answer: "Zero capacity on a work center is almost always one of six configuration gaps: an instance count of zero, no shift assigned, a shift whose weekdays all have zero hours, calendars (holidays or downtime) blanketing the window, a utilization percentage at or near zero, or the work center itself deactivated. Real capacity exhaustion, where every day is genuinely booked, is the rare seventh case."
...
qaQuestions:
  ...
  - question: "The failure dialog says no capacity within the search window for our deburr station, but the load view shows it half empty. Both cannot be true, can they?"
    answer: "They can, because they read different layers. The load view shows booked hours against configured shift time; the scheduler also applies utilization and calendars. A utilization percentage set to 10 leaves 90% of the visible shift time unusable to the scheduler, and a recurring downtime window can erase specific weekdays entirely. Check the station's utilization setting first (10 instead of 100 is a common data-entry slip), then its downtime records. The load view will keep looking half empty; the scheduler is the one telling the truth about what it may use."
---
...
Holidays (plant-wide, work-center-specific, and shift-level) and downtime events all subtract from capacity before anything schedules. A recurring downtime record accidentally set to cover every weekday, or a holiday range entered with the wrong year, can erase weeks at a stroke. Review the work center's calendar entries for the search window; the [holiday layers post](/blog/job-scheduled-on-a-holiday-edgebic) covers each type and its typical failure story.
