...
**Create shifts first.** A standard 8-hour day shift takes a minute: name it, set the daily start and end times per weekday, and set the break. Add a night shift the same way. Then handle the exceptions: plant-wide holidays (single or recurring annual), shift-specific holidays, and downtime events for planned maintenance.
...
5. **The breakdown response.** Record downtime on the work center, reschedule affected jobs, communicate the new dates: minutes, not hours, because the engine does the cascading.
