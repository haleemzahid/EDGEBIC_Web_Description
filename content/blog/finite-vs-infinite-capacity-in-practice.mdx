...
This is why finite dates are later than infinite dates and also why they are trustworthy. The finite engine is not being pessimistic. It is showing you the queue that already exists on your floor, the one infinite planning hides.

## A worked example: three jobs, one work center

One work center, one instance, one 8-hour day shift. To keep planning realistic you set the schedule-at-utilization to 80 percent, so the engine plans 6.4 productive hours per day rather than a fictional 8. Three jobs arrive Monday morning, each needing 10 run hours:
