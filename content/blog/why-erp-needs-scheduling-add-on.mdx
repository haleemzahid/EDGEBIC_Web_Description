...
wordCount: 2000
readingTime: 10
heroImage: "/marketing/blog/why-erp-needs-scheduling-add-on/hero.webp"
...
The answer that has held up across 35 years of these projects is to make the interface a file rather than a connector. Two shapes cover essentially every ERP:

- **File exchange** for any system that can export to Excel or CSV, which is all of them, including cloud platforms and legacy systems on strict IT policies
- **Database read** for ERPs with an accessible database, which covers most on-premise systems

Either way the mapping is the same object: a saved import mask that remembers which of your columns feeds which scheduling field. You map it once and re-run it. Work orders, routings, work centers, and due dates flow into the scheduler, and the finished plan exports back for return to the ERP. In EDGEBIC that flow can be put on a timer or tied to a watched file, so it runs unattended with a full run history rather than someone remembering to press import. For a deeper dive into integration methods, see our guide on [ERP Data Integration Best Practices](/blog/erp-data-integration-best-practices) and [how EDGEBIC import masks work](/blog/edgebic-import-masks-explained).

What this deliberately avoids is a per-ERP connector. There is no certification step before you can begin, no custom API development on the critical path, and nothing that breaks when your ERP vendor ships its next major release.
