...
**Fix:** fill the missing cell in the source file and re-import. A re-import reconciles rather than duplicates: existing rows are reused or updated, so re-running the whole corrected file does not double up the rows that already succeeded. [How to run an import](/blog/how-to-build-an-import-mask-in-edgebic) covers reading the result dialog and opening the log.
