...
**Capable-to-Promise (CTP)** is an advanced order promising method that evaluates whether a manufacturer has the capacity, materials, and time to produce an order that exceeds current [Available-to-Promise (ATP)](/blogs/glossary-available-to-promise) quantities. While ATP checks existing inventory and scheduled production, CTP simulates an unplanned production run to determine the earliest realistic delivery date.
...
1. **Check material availability** — Are all [BOM](/blogs/glossary-bill-of-materials) components in stock? If not, when could they arrive based on supplier lead times and current [purchase orders](/blogs/glossary-purchase-order)?
...
- **[Available-to-Promise (ATP)](/blogs/glossary-available-to-promise)** — The simpler promising method that CTP extends when existing stock and production are insufficient.
- **[Capacity Requirements Planning (CRP)](/blogs/glossary-crp)** — The capacity validation that CTP performs in real-time for individual order inquiries.
- **[Planning Horizon](/blogs/glossary-planning-horizon)** — The time span across which CTP evaluates material and capacity availability.
