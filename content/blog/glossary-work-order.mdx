...
A **work order** (also called a production order or manufacturing order) is an authorized document that instructs the shop floor to produce a specified quantity of an item by a certain date. In [MRP](/blog/glossary-material-requirements-planning)-driven manufacturing, work orders originate as [planned orders](/blog/glossary-planned-order) that a planner reviews and releases for execution.
...
### 1. Creation
MRP generates a planned order when [net requirements](/blog/glossary-net-requirements) are positive. The planner reviews the planned order, verifies material availability and capacity, and releases it as a work order.

### 2. Material Allocation
Upon release, the system allocates (reserves) components from inventory per the [BOM](/blog/glossary-bill-of-materials). If materials are short, the work order may be held until [scheduled receipts](/blog/glossary-scheduled-receipt) arrive.
...
**They capture actual performance data.** Work order completions record actual setup times, run times, quantities, and scrap. This data feeds back into planning parameters, improving future schedule accuracy — the essence of [closed-loop MRP](/blog/glossary-closed-loop-mrp).
...
- **[Planned Order](/blog/glossary-planned-order)** — The MRP recommendation that becomes a work order when released by the planner.
- **[Purchase Order](/blog/glossary-purchase-order)** — The external procurement counterpart to an internal work order.
- **[Yield](/blog/glossary-yield)** — The ratio of good output to total input on a work order, used to adjust future order quantities.
