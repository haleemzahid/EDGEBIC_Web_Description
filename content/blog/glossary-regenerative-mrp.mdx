...
**Regenerative MRP** is an [MRP](/blogs/glossary-material-requirements-planning) processing method that completely replans every item in the product database during each run. It deletes all existing [planned orders](/blogs/glossary-planned-order), re-explodes every [BOM](/blogs/glossary-bill-of-materials), recalculates all [net requirements](/blogs/glossary-net-requirements), and generates a fresh set of planned orders from scratch. It is the most thorough — but also the most computationally intensive — form of MRP processing.

At [User Solutions](/) we recommend regenerative MRP as the weekly "clean sweep" that ensures plan integrity, complemented by [net change MRP](/blogs/glossary-net-change-mrp) for daily responsiveness.
...
4. **Calculate [gross requirements](/blogs/glossary-gross-requirements)** for every component in every [time bucket](/blogs/glossary-time-bucket).
5. **Net against inventory and [scheduled receipts](/blogs/glossary-scheduled-receipt)** for every item.
6. **Apply [lot sizing](/blogs/glossary-lot-sizing)** and lead-time offsets to generate new planned orders.
...
- **[Net Change MRP](/blogs/glossary-net-change-mrp)** — The selective MRP method that replans only items affected by recent transactions, offering faster but less comprehensive updates.
- **[Planned Order](/blogs/glossary-planned-order)** — The order recommendation that regenerative MRP creates fresh during each complete run.
- **[Closed-Loop MRP](/blogs/glossary-closed-loop-mrp)** — The planning framework that uses regenerative or net change MRP as its core planning engine.
