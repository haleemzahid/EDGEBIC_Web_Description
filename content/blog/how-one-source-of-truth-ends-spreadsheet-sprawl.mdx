...
The problem is that nothing keeps them in sync except memory and discipline. Move a date in the master and the capacity tab, the shipping list, and the planner's copy are all now stale until someone updates them by hand. The moment one gets missed, the shop is running on two different truths, and nobody can say which sheet is right without checking against the floor. The sprawl is not a filing problem. It is a correctness problem, and it compounds every time the plan changes.
