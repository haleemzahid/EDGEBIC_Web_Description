...
Always plan against effective capacity. Using theoretical or practical capacity leads to overloaded schedules and missed deliveries — the same problem [infinite capacity planning](/blogs/finite-vs-infinite-capacity-planning) creates.
...
A ratio above 1.0 means demand exceeds capacity. A ratio of 0.85-0.90 is optimal for [constraint resources](/blogs/resource-constraints-manufacturing).
...
For [CNC machine scheduling](/blogs/cnc-machine-scheduling), the capacity formula must account for the reality that one CNC mill is not identical to another — different machines have different capabilities, tooling packages, and accuracy levels.
...
Assembly capacity is often [labor-constrained](/blogs/labor-capacity-planning) rather than equipment-constrained. The workbench does not limit throughput — the assembler does. Capacity planning for assembly should focus on labor hours and skill requirements rather than station hours.
...
Manufacturers who apply SMED to their [constraint resources](/blogs/resource-constraints-manufacturing) typically reduce setup times by 40-60%, recovering significant capacity from existing equipment.
...
If the gap is persistent and growing, capital investment is justified. If it is seasonal or temporary, [overtime, outsourcing, or shift changes](/blogs/seasonal-capacity-planning) may suffice.

### Medium-Range (1-3 months): [Rough-Cut Capacity Planning](/blogs/rough-cut-capacity-planning)
...
This is where [finite capacity planning](/blogs/what-is-finite-capacity-planning) operates. Every job is loaded against every machine in time sequence. The schedule shows exactly when each machine will run each job, identifies which machines are fully loaded, and calculates realistic completion dates.
...
The [Theory of Constraints](/blogs/theory-of-constraints-scheduling) principle applies: invest only at the constraint. Adding capacity to a non-bottleneck machine does not increase total factory output by a single unit.
...
At that complexity level, you need [capacity planning software](/blogs/capacity-planning-software) that:
...
RMDB provides all of these capabilities with implementation in as few as [5 business days](/blogs/5-day-implementation-framework). The system defines your machines, loads your orders, and produces your first finite capacity schedule within the first week.
...
As discussed in our guide to [capacity utilization rates](/blogs/capacity-utilization-rate), maximizing utilization on non-constraint machines creates WIP and extends lead times without improving throughput.
