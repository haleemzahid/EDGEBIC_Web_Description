...
**To add a sub-assembly to a routing in EDGEBIC by User Solutions you drag the component onto the Designer canvas, link it to the step that consumes it, and set its per-unit quantity.** The component becomes a sub-assembly automatically, because it carries a routing of its own, and the scheduler plans that routing whenever the parent product is scheduled, as long as [the include sub-assemblies policy](/blog/how-to-include-sub-assemblies-in-the-schedule-in-edgebic) is left on, which it is by default.
