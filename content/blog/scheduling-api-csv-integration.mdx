...
**Where EDGEBIC sits.** EDGEBIC by User Solutions works from the CSV, Excel, and database side of this comparison rather than shipping ERP APIs of its own. A saved import mask holds the column mapping, and an integration definition can re-run it on an interval or the moment a watched file changes, which recovers most of the automation people want from an API without anyone owning a piece of ERP-specific code. See [how EDGEBIC import masks work](/blog/edgebic-import-masks-explained) and [scheduled integration runs](/blog/edgebic-scheduled-integration-runs-explained).
