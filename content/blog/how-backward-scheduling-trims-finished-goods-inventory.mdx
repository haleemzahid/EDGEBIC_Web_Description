...
Backward scheduling reverses the logic. It starts from the due date and works backward through the routing, operation by operation, subtracting each step's run time, queue, and any transit until it finds the latest start that still meets the date. The job then completes near the due date, not weeks ahead of it, and because the first operation also starts later, [raw material is needed later too](/blog/how-backward-scheduling-delays-raw-material-staging) and stays in the stockroom instead of on the floor. See [EDGEBIC backward scheduling explained](/blog/edgebic-backward-scheduling-explained) for the step-by-step.
