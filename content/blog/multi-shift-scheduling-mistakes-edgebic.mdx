...
description: "The multi shift scheduling mistakes that break capacity in EDGEBIC: the 50 percent utilization default, empty shifts, global shift overrides, and six more."
...
faqQuestions:
  ...
  - question: "Why is my capacity half what I expect in EDGEBIC?"
    answer: "Work center utilization defaults to 50 percent, and it multiplies the whole capacity calculation. A two-instance work center on an 8-hour shift offers 16 nominal hours but only 8 usable hours until utilization is raised. Set it per machine from what that work center genuinely converts to production, rather than accepting the default across the plant."
...
## 1. Leaving Work Center Utilization at 50 Percent

**Symptom:** Every work center delivers about half the capacity you expect. Jobs run twice as long as your own estimate. The plant looks permanently overloaded.

**Cause:** Utilization defaults to 50 percent and multiplies the entire capacity calculation. A two-instance work center on an 8-hour shift shows 16 nominal hours and offers 8 usable ones.

**Fix:** Set utilization per work center, deliberately. Ask what fraction of the nominal shift each machine genuinely converts into production once breaks, minor stoppages, and unrecorded interruptions are removed. Well-run equipment usually lands between 80 and 90 percent. Do this before your first real schedule rather than after three weeks of confusing results.

This is the single most common setup error, and it is silent: nothing warns you, the schedule is internally consistent, and it is simply half as productive as your shop.
...
1. **Utilization.** Is it still 50 percent anywhere? Every work center should carry a number somebody chose.
...
The order matters. Utilization and instance count set the size of every slot, so a mistake there distorts everything downstream and is invisible in the schedule itself. Shift assignment errors are noisier and easier to spot. Calendar gaps announce themselves eventually, usually as a job scheduled on a day the plant is closed.
