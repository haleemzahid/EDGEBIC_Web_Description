...
Every entry is stamped with a bin location, taken from the product's default location at the moment of posting. Set that on the product editor, in the same inventory planning section as the build method and reorder settings. The default is a generic location, so a shop that never sets it will see the same string on every row, which is harmless.
