...
A finding that disappears on a fresh run was stale. One that persists is worth the attention its severity implies, and when a row survives a correction you know you made, [an anomaly row that outlives the fix](/blog/the-anomaly-report-still-lists-a-problem-i-fixed-edgebic) explains why the report reads stored state. When several checks fire on one job, work the Critical rows first: fixing the root often clears the dependent Warnings automatically, which is the pattern behind [why a job jumps after a reschedule](/blog/why-did-my-job-jump-after-a-reschedule-edgebic). The [troubleshooting guide](/blog/edgebic-troubleshooting-guide) links the specific checks.
