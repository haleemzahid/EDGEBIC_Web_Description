...
faqQuestions:
  ...
  - question: "Why did a pieces-based work center produce impossible durations?"
    answer: "Almost certainly because the pieces-per-hour rate is zero. With the capacity type set to pieces and no rate on the routing step, there is nothing to divide the quantity by, and the resulting durations are meaningless. A pieces-based machine must always carry a non-zero rate. It is the single most common configuration error on this feature and it is easy to spot because the durations are absurd rather than subtly wrong."
  - question: "Does the unit of measure control whether a work center schedules in pieces?"
    answer: "No. A unit of measure carries a scheduling behavior that acts as a hint and a default when work centers are built around that unit, but the work center carries its own capacity type independently and that is what the engine reads. If the two disagree, the work center wins. Both appear as read-only columns on the work centers grid, so read them there rather than assuming the machine inherited from the unit."
...
The second is always available shift hours, minus downtime and partial holidays, multiplied by the instance count, multiplied by the utilization percentage. Work centers run at their full clock in current versions, so an eight-hour shift on a three-instance machine offers 24 hours. That does not change because you switched to pieces. [How to configure instances and utilization](/blog/how-to-configure-instances-and-utilization-in-edgebic) covers that half.
...
|---|---|---|
| Utilization percentage | Work center | Yes, the sole multiplier in the shift capacity formula. Real, but it runs at the full clock and is not edited on the work center screen |
| Efficiency factor | Work center | Yes, on the pieces path. Multiplies pieces per hour |
| Efficiency | Work center | No. Not the capacity throttle, and changing it does not change scheduled hours |
| Factor | Machine pool membership | Yes, but on a different axis: it scales run hours for that pool member, and is never applied to setup |
