...
Change nothing but MO-101's priority to 1 and it jumps ahead of MO-102 only if its start date is earlier or equal and its due date wins the tie; otherwise the two priority-1 jobs sort between themselves by start date. The rule composes cleanly, one key at a time.
