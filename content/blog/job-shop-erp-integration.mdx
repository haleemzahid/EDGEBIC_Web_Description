...
**Note on EDGEBIC:** EDGEBIC by User Solutions integrates via Methods 1 and 2, file exchange and a database read, not via per-ERP APIs. That is why the answer to "will it work with our ERP" is almost always yes: if your system can export a file or expose a readable database, the mapping is the same job regardless of which ERP it is.
