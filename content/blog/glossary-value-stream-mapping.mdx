...
**Value stream mapping (VSM)** is a lean manufacturing visual analysis tool that documents the end-to-end flow of materials and information required to deliver a product to the customer. VSM reveals waste that traditional process charts miss by showing not just the operations but the queues, inventory buffers, information flows, and time delays between them. This [manufacturing glossary](/blogs/manufacturing-glossary) entry explains how VSM works, shows real results, and connects it to scheduling improvement.

## What Is Value Stream Mapping?

VSM is a pencil-and-paper exercise (intentionally low-tech) that maps the entire [value stream](/blogs/glossary-value-stream) for a product family using standardized symbols:
...
Create a redesigned value stream that eliminates waste: establish [flow](/blogs/glossary-flow-production) where possible, implement [pull](/blogs/glossary-pull-system) where flow is not possible, level the schedule with [Heijunka](/blogs/glossary-heijunka), and size supermarkets to buffer necessary variation.

### Step 6: Create the Implementation Plan
Break the future state into achievable improvement loops. Each loop is a focused project — a [Kaizen](/blogs/glossary-kaizen) event, a cell redesign, a Kanban implementation — with a timeline and owner.
...
- Implemented [Kanban](/blogs/glossary-kanban) between the cell and downstream assembly
- Reduced batch sizes by 60% after [SMED](/blogs/glossary-single-minute-exchange) events on two bottleneck machines
...
The [lean manufacturing guide](/blogs/lean-manufacturing-guide) positions VSM as the diagnostic tool that precedes all other lean improvements — you must see the waste before you can eliminate it.

## Related Terms

- **[Value Stream](/blogs/glossary-value-stream)** — The end-to-end flow of activities that VSM documents and analyzes.
- **[Waste Elimination](/blogs/glossary-waste-elimination)** — The systematic removal of non-value-added activities that VSM identifies.
- **[Kaizen](/blogs/glossary-kaizen)** — The focused improvement events used to implement changes identified through VSM analysis.
