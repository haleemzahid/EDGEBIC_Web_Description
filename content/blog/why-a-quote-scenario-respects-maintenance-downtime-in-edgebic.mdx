...
faqQuestions:
  - question: "Does a quote scenario in EDGEBIC account for scheduled maintenance?"
    answer: "Yes. A scenario simulation in EDGEBIC by User Solutions reads the same per-day capacity overrides and holiday calendars that the production scheduler uses. So a machine zeroed for maintenance next Tuesday, a plant holiday, or a day set to reduced capacity all show up in the what-if. The scenario runs against your real, constrained calendar, not an idealized one, which is what makes its quoted date trustworthy."
  - question: "Where does the scenario get its calendar constraints from?"
    answer: "From the same configuration the live schedule uses. Maintenance, holidays, and per-day capacity changes are set once, as plant holidays and per-day capacity overrides, and both the production scheduler and the scenario simulation read them. There is no separate quoting calendar to maintain. When a planner marks a work center down for a day or lowers a shift's capacity, every future scenario reflects it automatically without any extra step."
