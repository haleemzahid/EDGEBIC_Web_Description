...
**Statistical Process Control (SPC)** is a method of quality control that uses statistical techniques to monitor, control, and improve manufacturing processes. The core tool of SPC is the [control chart](/blogs/glossary-control-chart), which plots process measurements over time against statistically calculated [control limits](/blogs/glossary-control-limits) to distinguish between normal variation (common cause) and abnormal variation (special cause).
...
**Data Collection.** Measurements are collected from the process at regular intervals. Data can be [variable data](/blogs/glossary-variable-data) (continuous measurements like diameter, weight, or temperature) or [attribute data](/blogs/glossary-attribute-data) (classifications like pass/fail or defect counts).

**Control Chart Selection.** The appropriate chart type is selected based on the data type and sampling approach. [X-bar and R-charts](/blogs/glossary-x-bar-chart) for variable data with subgroups, individuals charts for single measurements, [p-charts](/blogs/glossary-p-chart) for proportion defective, and [c-charts](/blogs/glossary-c-chart) for defect counts.
...
**Reaction to Signals.** When an out-of-control signal is detected, the operator follows a defined response procedure: stop the process if necessary, inspect recent output, investigate the assignable cause, implement [corrective action](/blogs/glossary-corrective-action), and verify the process returns to control.

**Capability Analysis.** Once the process is in statistical control, [capability indices](/blogs/glossary-capability-index) (Cp, Cpk) are calculated to determine whether the process can meet the engineering [specification limits](/blogs/glossary-specification-limits). A process can be in control but not capable — meaning it is predictably producing some out-of-specification parts.
...
- [Control Chart](/blogs/glossary-control-chart) — the primary monitoring tool used in SPC
- [Control Limits](/blogs/glossary-control-limits) — the statistically calculated boundaries on SPC control charts
- [Capability Index](/blogs/glossary-capability-index) — the measure of how well a controlled process meets specifications
