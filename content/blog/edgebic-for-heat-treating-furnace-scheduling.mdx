...
A furnace calendar is only as good as the cycle times behind it. Operators log actual load and unload on a shop-floor kiosk, so a cycle that ran long because of a slow ramp or an extended soak is captured as a measured duration rather than an assumption. Those actuals flow back into the same schedule that planned the loads, and a reschedule around a delayed cycle never moves a load that already ran, it only re-times the loads still waiting. The workflow is described in [shop-floor actuals tracking](/blog/edgebic-for-machine-shops-actuals-tracking).
...
For a related batch scheduling application, see [foundries and casting lot scheduling](/blog/edgebic-for-foundries-and-casting-lot-scheduling), and for the fundamentals, [what is production scheduling](/blog/what-is-production-scheduling). The [industry fit guide](/blog/edgebic-by-industry) maps the rest, and [EDGEBIC](/edgebic) is the product hub.
