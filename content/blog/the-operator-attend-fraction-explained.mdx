...
The operator attend fraction is how EDGEBIC captures the real staffing shape of a machine: setup-only for a lights-out cell, a share for multi-machine tending, one for a fully attended run, or more than one for a crew. Setup always books one person, only the run scales, and the zero default keeps a forgotten setting from silently overselling your people. Set it deliberately per step and the schedule respects the second constraint that a machine-only plan ignores. Explore the whole execution loop in the [shop floor execution guide](/blog/edgebic-shop-floor-guide), see how the plan reflects real [production bottleneck identification](/blog/production-bottleneck-identification), and visit [EDGEBIC](/edgebic).
