...
These costs are why manufacturers invest in [safety stock](/blogs/glossary-safety-stock-glossary), [reorder points](/blogs/glossary-reorder-point), and demand planning systems — the cost of preventing a stockout is almost always less than the cost of experiencing one.
...
**Demand exceeds forecast.** Customer orders or production requirements are higher than predicted. If [safety stock](/blogs/glossary-safety-stock-glossary) is insufficient to absorb the excess demand, a stockout occurs.
...
**Quality rejections.** Incoming material fails [inspection](/blogs/glossary-inspection), reducing the usable quantity below the expected level. The rejected material cannot enter production, and a replacement shipment takes additional lead time.
...
Manufacturers measure stockout performance through the [service level](/blogs/glossary-service-level) metric — the percentage of demand fulfilled from available stock. A 95% service level means stockouts occur in 5% of replenishment cycles.
...
After this event, the manufacturer adjusts the [reorder point](/blogs/glossary-reorder-point) for impeller housings, increasing safety stock from 15 to 40 units and tightening the supplier delivery monitoring process.
...
- [Safety Stock](/blogs/glossary-safety-stock-glossary) — buffer inventory specifically designed to prevent stockouts
- [Reorder Point](/blogs/glossary-reorder-point) — the trigger level that initiates replenishment before a stockout occurs
- [Service Level](/blogs/glossary-service-level) — the metric that measures how successfully stockouts are being prevented
