...
qaQuestions:
  ...
  - question: "Our percent complete showed almost zero on a job we knew was nearly done. What causes that?"
    answer: "A pieces fallback firing when it should not. Percent complete is hours-first: if planned and actual hours both exist, the figure is actual over planned. Only when no hours are logged does it fall back to a pieces ratio, and on a one-piece job an auto-calculated fractional piece count produces a number near zero, which then reads as a job that has barely started. The fix is upstream and simple: log hours. Hours-primary matching across the job view, the earned value report, and the end-product roll-up is what keeps all three telling the same story, and a job with no logged hours will always be guessed at rather than measured."
---
...
Every column below carries its own definition inside the product, one click away on the Column Details button, which is covered in [how to use Column Details](/blog/how-to-use-column-details-in-edgebic). The reports themselves are in the [catalogue overview](/blog/edgebic-reports-explained).
...
## 2. Hours-First, Pieces-Fallback

Percent complete is hours-primary by design. If planned and actual hours exist, the figure is the hours ratio. Only when no hours are logged does it fall back to a pieces ratio against the order quantity.
