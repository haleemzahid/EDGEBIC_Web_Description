...
description: "When a Gantt bar spans far more calendar time than the work hours, the cause is a low utilization cap on the order, high queue time, a large setup charge, or shifts spread thin."
...
faqQuestions:
  - question: "Why is a job's schedule bar much wider than its total work hours?"
    answer: "A bar spans more calendar time than its work hours for one of four reasons: a low schedule-at-utilization cap on the order stretches a few hours of work across many days, a large queue time inserts non-work buffer between steps, a sequence-dependent setup charge adds real changeover hours, or the job spans many shifts with small daily allocations. The width is real elapsed time, not wasted capacity, once you find the factor."
  ...
  - question: "How do I tell the difference between a wide bar and an idle gap?"
    answer: "A wide bar is a single operation whose window is stretched by a low utilization cap or many small daily allocations. An idle gap is empty time between two separate operations, flagged by the capacity-waste check when it exceeds half an hour. Check whether the width is inside one operation's bar (the cap or setup) or between two bars (queue, flow, transit, or genuine idle time). The two have different fixes."
qaQuestions:
...
**Fix:** if the cap is a data-entry slip (10 or 25 entered where 100 was meant), correct it on the order and re-run scheduling. See [the schedule-at-utilization setting](/blog/what-is-the-schedule-at-utilization-setting). If the low number is deliberate, because the machine is genuinely shared and you wanted headroom held open, the wide bar is correct and reflects the real pace.
...
2. **Check the order's utilization cap.** A low percentage is the single most common stretcher.
