...
wordCount: 2000
readingTime: 10
heroImage: "/marketing/blog/oracle-scheduling-integration/hero.webp"
...
**All Platforms:**
- CSV/Excel file import/export, which works everywhere
- Direct read access where the Oracle database is reachable

These last two are the ones User Solutions actually uses. Rather than maintaining separate connectors for Oracle Cloud, EBS, and JD Edwards, EDGEBIC by User Solutions maps whatever those platforms export through a saved import mask, so all three integrate the same way and nothing has to be re-certified when Oracle ships an upgrade.
