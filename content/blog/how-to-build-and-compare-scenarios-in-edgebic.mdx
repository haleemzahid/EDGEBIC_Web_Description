...
faqQuestions:
  ...
  - question: "Which scenarios can be compared against each other?"
    answer: "Simulated scenarios belonging to the same parent quote. Tick the ones you want on the comparison tab and three charts fill in: lead time in days, cost, and resource utilisation, with one bar per scenario. An unsimulated scenario cannot join a comparison, which stops a variant that was never run from appearing as a zero."
...
## Modelling the Situations That Actually Come Up
...
Read all three. The second shift costs $240 more than the weekend push and buys three days of margin. The utilisation chart is where you check whether the winning scenario runs a machine so hard that a single breakdown loses the date anyway.
...
4. Run each one.
5. Compare lead time, cost, and utilisation together.
