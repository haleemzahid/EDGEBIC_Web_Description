...
High attribute defect rates on specific operations may indicate that more time needs to be allocated for inspection, or that certain work centers need maintenance before the next production run. Scheduling tools like [Resource Manager DB](/resource-manager-db-2/) allow planners to build inspection time into operation sequences and adjust schedules when quality data indicates process problems.
