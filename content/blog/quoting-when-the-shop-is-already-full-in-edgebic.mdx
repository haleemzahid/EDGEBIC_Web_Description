...
A scenario also carries a priority level, and [raising it lets the simulated job claim contested capacity sooner](/blog/how-scenario-priority-level-changes-the-quoted-date-in-edgebic). Each scenario re-simulates and stores its own date and cost, so you can line them up. The full mechanism is in [comparing routing options for a quote](/blog/comparing-routing-options-for-a-quote-in-edgebic) and [EDGEBIC what-if scenarios explained](/blog/edgebic-what-if-scenarios-explained).
