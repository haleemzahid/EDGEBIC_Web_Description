...
**Earliest start** picks the machine that can begin the step soonest, which favors getting the job moving over finishing it soonest. It is useful when starting early unblocks downstream work, though note it evaluates start time at day granularity, so a machine that can start today beats one that can only start tomorrow even if the second would finish sooner.
