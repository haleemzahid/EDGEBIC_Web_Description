...
faqQuestions:
  - question: "What is Budget at Completion (BAC)?"
    answer: "Budget at Completion is the total planned work for a job, captured at the moment the job was scheduled and used as the fixed baseline for every earned value comparison that follows. EDGEBIC measures it in planned hours by summing every step's planned time, and in planned cost where labor and overhead rates are entered. Because BAC never moves once set, it is the honest yardstick against which actual hours and forecast overruns are judged."
  - question: "Is BAC measured in hours or dollars?"
    answer: "In EDGEBIC's earned value report, BAC is expressed primarily in planned hours because hours are the currency the scheduling engine works in. When you have entered labor rates and overhead, the same baseline also carries a planned cost. A five-step job planned at 8, 12, 24, 4, and 2 hours has a BAC of 50 hours, and that 50-hour figure anchors the schedule performance index, the cost performance index, and the estimate at completion."
...
BAC appears as a column in EDGEBIC's Earned Value report, sitting beside earned value, planned value, actual cost, the two performance indices, and the estimate at completion. You reach it from the reports catalogue and run it for a date range or a single job. See [how to run the earned value report in EDGEBIC](/blog/how-to-run-the-earned-value-report-in-edgebic) for the step by step.
