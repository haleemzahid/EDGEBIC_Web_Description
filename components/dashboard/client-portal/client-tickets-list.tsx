...
export type ClientTicketsListProps = {
...
export function ClientTicketsList({
  tickets,
  filteredCount
}: ClientTicketsListProps): React.JSX.Element {
  const router = useRouter();
  const { isLoading, startTransition } = useTransitionContext();

  // Auto-refresh so team replies and status changes appear without a manual reload.
...
function ClientTicketRow({
  ...
  return (
    <li>
      <Link
        href={`${Routes.ClientSupport}/${ticket.id}`}
