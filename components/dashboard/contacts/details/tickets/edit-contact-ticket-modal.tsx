...
import {
  updateContactTicketSchema,
  type UpdateContactTicketSchema
} from '@/schemas/contacts/update-contact-ticket-schema';
...
export type EditContactTicketModalProps = NiceModalHocProps & {
...
export const EditContactTicketModal =
  NiceModal.create<EditContactTicketModalProps>(
    ({ ticket, members = [], meetings = [] }) => {
      ...
      const methods = useZodForm({
        ...
        defaultValues: {
          ...
          meetingId: ticket.meetingId ?? null
        }
      });
      const title = `Edit ticket #${ticket.number}`;
      const description = 'Update ticket details visible to your team and the customer.';
      const canSubmit =
        !methods.formState.isSubmitting &&
        (!methods.formState.isSubmitted || methods.formState.isDirty);
      const onSubmit: SubmitHandler<UpdateContactTicketSchema> = async (
        values
      ) => {
        if (!canSubmit) {
          return;
        }
        const result = await updateContactTicket(values);
              ...
              <FormItem className="flex w-full flex-col space-y-1.5">
                ...
                <FormControl>
                  ...
                  >
                    ...
                    <SelectContent>
                      <SelectItem value={AUTO_ASSIGN}>Unassigned</SelectItem>
                      {members.map((m) => (
                        <SelectItem
                          key={m.id}
                          value={m.id}
